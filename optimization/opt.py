...
sys.path.insert(0,'/home/user/matrix-conics/CoordGeo') 
...
#Initial guess
h.value = 1; l.value = 45 - 2*h.value; b.value = 24 - 2*h.value
hk = h.value - 10*delta; lk = 45 - 2*hk; bk = 24 - 2*hk
...
while(np.abs(h.value - hk) > delta and num_iter < max_iter):
    ...
    constraints = [ceq1 == 45, ceq2 == 24, h >= hk/(1+error) , h <= min([(1+error)*hk, 1.5])]
